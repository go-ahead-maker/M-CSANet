...
class DWConv2d_BN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pwconv(x)
        x = self.act(x)
        if isinstance(self.bn, nn.LayerNorm):
            B, C, H, W = x.shape
...
class DWCPatchEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_conv = DWConv2d_BN(in_chans,embed_dim, kernel_size=patch_size,
                                      stride=stride, act_layer=act_layer, norm_layer=norm_layer
                                      )
...
class Patch_Embed_stage(nn.Module):
    ...
    def __init__(self, embed_dim, num_path=4, isPool=False, is_attn=False):
        ...
        if is_attn:   # in front of the HSSA stages
...
class LCAModel(nn.Module):
    def __init__(self,
        ...
        self.norm_adpt = nn.BatchNorm2d(dim // 4)
    ...
    def forward(self, x):
        ...
        attn = self.norm_attn(attn)
        attn = self.dw_d_conv(attn)

        adaptive = self.act(self.proj1(adaptive))
        adaptive = self.proj2(self.norm_adpt(adaptive))
...
class ChannelWiseAttention(nn.Module):
    ...
    def __init__(
        ...
        self.cpe = ConvPosEnc(dim=num_channel_tokens,)
    ...
    def forward(self, x):
        ...
        x = self.cpe(x, (int(np.sqrt(N)), int(np.sqrt(N))))
...
class LocalConvAttnention(nn.Module):
    ...
    def forward(self,x):  # B,C,H,W
        ...
        x_norm = self.norm1(x)
        x = x + self.drop_path(self.dynamicdwconv(x_norm))

        x_norm = self.norm2(x)
        x = x + self.drop_path(self.CMLP(x_norm))
        return x
...
class MPLocalConvAttnStage(nn.Module):
    def __init__(self,
        ...
        self.channel_attn_norm1 = norm_layer(embed_dim)
        self.channel_attn_norm2 = norm_layer(embed_dim)
...
class CAMixer(nn.Module):
    ...
    def __init__(
        ...
        in_chans=3,
        num_classes=1000,
        attn_category = [-1, 'factorized', -1, 'factorized'],
        dw_kernel = [5, -1, 5, -1],
        dw_d_kernel = [7, -1, 3, -1],
        num_groups=[2, -1, 4, -1 ],
        use_chk=False,
